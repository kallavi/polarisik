...
@section('rightContent')
    <x-admin.tags-wrapper :class="'d-flex align-items-center'">
        <x-slot:tagsWrapper>
            ...
            <x-admin.button :link="route('admin.video.create')" :title="'Yeni Ekle'" :class="'d-flex flex-center h-48px h-lg-48px'" :color="'primary'" :iconTag="'i'" :iconClass="'fs-2x me-2'" :iconType="'outline'"
                :iconName="'add-files'" />
            ...
            <x-admin.data-table :search="''" :export="''" :searchNone="''" :exportNone="''">
                ...
                <x-slot:rows>
                    @foreach ($videos as $video)
                        <tr>
                            <td>
                                <x-admin.img-wrapper :imgPath="'/' . $video->image" />
                            <td>
                                {{ $video->{'name:tr'} }}
                            </td>
                            ...
                            <td class="text-end">
                                {{-- <x-admin.button :link="'/video/' . $video->image" :target="'_blank'" :class="'btn-icon'" :small="''" :light="''" :color="'success'"
                                    :iconTag="'i'" :iconClass="'fs-1'" :iconType="'outline'" :iconName="'eye'" :data="[
                                        'kt-menu-trigger' => 'click',
                                        'kt-menu-placement' => 'bottom-end',
                                        'kt-menu-overflow' => 'true',
                                    ]" /> --}}
                                <x-admin.button :link="route('admin.video.edit', $video->id)" :class="'btn-icon '" :small="''" :light="''" :color="'primary'" :iconTag="'i'"
                                    :iconClass="'fs-1'" :iconType="'outline'" :iconName="'message-edit'" :data="[
                                        'kt-menu-trigger' => 'click',
                                        'kt-menu-placement' => 'bottom-end',
                                        'kt-menu-overflow' => 'true',
                                    ]" />
                                <x-admin.form :id="'kt_ecommerce_add_product_form'" :class="'d-inline'" :action="route('admin.video.destroy', $video->id)" :method="'POST'">
                                    <x-slot:form>
                                        @method('DELETE')
