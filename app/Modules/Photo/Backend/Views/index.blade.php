...
@section('rightContent')
    <x-admin.tags-wrapper :class="'d-flex align-items-center'">
        <x-slot:tagsWrapper>
            ...
            <x-admin.button :link="route('admin.photo.create')" :title="'Yeni Ekle'" :class="'d-flex flex-center h-48px h-lg-48px'" :color="'primary'" :iconTag="'i'"
            ...
            <x-admin.data-table :search="''" :export="''" :searchNone="''" :exportNone="''">
                ...
                <x-slot:rows>
                    @foreach ($photos as $photo)
                        <tr>
                            <td>
                                <x-admin.img-wrapper :imgPath="'/' . $photo->image" />
                            <td>
                                {{ $photo->{'name:tr'} }}
                            </td>
                            ...
                            <td class="text-end">
                                <x-admin.button :link="'/photo/'  . $photo->image" :target="'_blank'" :class="'btn-icon'"
                                    ...
                                    <x-admin.button :link="route('admin.photo.edit', $photo->id)" :class="'btn-icon '" :small="''"
                                ...
                                <x-admin.form :id="'kt_ecommerce_add_product_form'" :class="'d-inline'" :action="route('admin.photo.destroy', $photo->id)" :method="'POST'">
                                    <x-slot:form>
                                        @method('DELETE')
