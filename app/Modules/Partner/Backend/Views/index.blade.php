...
@section('rightContent')
    <x-admin.tags-wrapper :class="'d-flex align-items-center'">
        <x-slot:tagsWrapper>
            ...
            <x-admin.button :link="route('admin.partner.create')" :title="'Partner Ekle'" :class="'d-flex flex-center h-48px h-lg-48px'" :color="'primary'" :iconTag="'i'"
        ...
        <x-slot:content>
            <x-admin.data-table :search="''" :export="''" :searchNone="''" :exportNone="''">
                <x-slot:columns>
                    ...
                    <th class="fw-bolder text-gray-600">URL</th>
                ...
                <x-slot:rows>
                    @foreach ($partners as $partner)
                        <tr>
                            <td>
                                <x-admin.img-wrapper :imgPath="'/' . $partner->image" />
                            <td>
                                {{ $partner->name }}
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                    <x-admin.button :link="route('admin.partner.edit', $partner->id)" :class="'btn-icon '" :small="''"
                                ...
                                <x-admin.form :id="'kt_ecommerce_add_product_form'" :class="'d-inline'" :action="route('admin.partner.destroy', $partner->id)" :method="'POST'">
                                    <x-slot:form>
                                        @method('DELETE')
